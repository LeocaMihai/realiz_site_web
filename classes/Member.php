...
class Member
{
    ...
    public function create($data)
    {
        $query = "INSERT INTO members 
            (first_name, last_name, email, profession, company, expertise, linkedin_profile, profile_picture, created_at) 
            VALUES (:first_name, :last_name, :email, :profession, :company, :expertise, :linkedin_profile, :profile_picture, CURRENT_TIMESTAMP)";
    ...
    public function getTotal($filterProfession = '', $filterCompany = '')
    {
        $query = "SELECT COUNT(*) as total FROM members WHERE profession LIKE :profession AND company LIKE :company ";
        $stmt = $this->db->prepare($query);
        $stmt->bindValue(':profession', "%$filterProfession%", PDO::PARAM_STR);
        $stmt->bindValue(':company', "%$filterCompany%", PDO::PARAM_STR);
